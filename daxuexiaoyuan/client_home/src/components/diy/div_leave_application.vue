<template>
	<div class="diy_details_box diy_div_leave_application">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "leave_application",
					source_field: "leave_application_id",
					source_id: this.obj["leave_application_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/leave_application/set?leave_application_id=' + _this.obj["leave_application_id"], {
								praise_len
							}, (res) => {
								if(res.result){
