<template>
	<div class="diy_details_box diy_div_travel_report">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "travel_report_id",
					source_id: this.obj["travel_report_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/travel_report/set?travel_report_id=' + _this.obj["travel_report_id"], {
								praise_len
							}, (res) => {
								if(res.result){
