<template>
	<div class="diy_home diy_list diy_leave_application" id="diy_leave_application_list">
		<!-- 列表 -->
		<div class="diy_view_list list list-x" v-if="show">
			<router-link class="diy_card goods diy_list_box_wrap" v-for="(o, i) in list" :key="i"
				:to="'/leave_application/details?leave_application_id=' + o['leave_application_id']">
				<!-- 图片 -->
				<div class="diy_list_img_box" v-if="imgList.length" >
					<div class="diy_row" v-for="(item,index) in imgList" :key="item+index" v-show="$check_field('get',item.name,'/leave_application/details') && +item.is_img_list">
				...
				<div class="diy_list_item_box">
					<div class="diy_list_item_content" v-for="(item,index) in showItemList" :key="item+index">
						<div class="diy_row" :class="{[item.name]:true}" v-if="$check_field('get',item.name,'/leave_application/details') && +item.is_img_list">
							<div class="diy_title">
								<span>{{item.title}}:</span>
							</div>
		...
		<div class="diy_view_table" v-else>
			<table class="diy_table">
				<tr class="diy_row">
					...
					<th class="diy_title" v-if="$check_field('get','actual_days')">实际天数
					</th>
					<th class="diy_title" v-if="$check_field('get','certificate_of_cancellation_of_leave')">销假凭证
					</th>
				</tr>
				<tr class="diy_row" v-for="(o,i) in list" :key="o+i">
					...
					<td class="diy_field diy_text" v-if="$check_field('get','actual_days')">
						<span>
							{{ o["actual_days"] }}
						</span>
					</td>
					<td class="diy_field" v-if="$check_field('get','certificate_of_cancellation_of_leave')">
						<img class="diy_img" :src="o['certificate_of_cancellation_of_leave']" />
					</td>
				</tr>
			</table>
		</div>
	...
	export default {
		...
		data() {
			return {
						...
						{
							title: "销假凭证",
							name: "certificate_of_cancellation_of_leave",
							type: "图片",
							is_img_list: "0"
						},
						],
