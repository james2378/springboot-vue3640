<template>
	<div class="diy_details_box diy_div_leave_management">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "leave_management",
					source_field: "leave_management_id",
					source_id: this.obj["leave_management_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/leave_management/set?leave_management_id=' + _this.obj["leave_management_id"], {
								praise_len
							}, (res) => {
								if(res.result){
