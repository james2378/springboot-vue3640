		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','vaccine_name') || $check_field('add','vaccine_name') || $check_field('set','vaccine_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="疫苗名称" prop="vaccine_name">
					<el-input id="vaccine_name" v-model="form['vaccine_name']" placeholder="请输入疫苗名称"
							  v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','vaccine_name')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','vaccine_name'))" :disabled="disabledObj['vaccine_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','inoculation_point') || $check_field('add','inoculation_point') || $check_field('set','inoculation_point')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="接种点" prop="inoculation_point">
					<el-input id="inoculation_point" v-model="form['inoculation_point']" placeholder="请输入接种点"
							  v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','inoculation_point')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','inoculation_point'))" :disabled="disabledObj['inoculation_point_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_job_number') || $check_field('add','teacher_job_number') || $check_field('set','teacher_job_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="教师工号" prop="teacher_job_number">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','teacher_job_number')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','teacher_job_number'))" :disabled="disabledObj['teacher_job_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','teacher_job_number')">{{form['teacher_job_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','teacher_job_number')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','teacher_job_number'))" id="teacher_job_number" v-model="form['teacher_job_number']" :disabled="disabledObj['teacher_job_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','faculty') || $check_field('add','faculty') || $check_field('set','faculty')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="院系" prop="faculty">
					<el-input id="faculty" v-model="form['faculty']" placeholder="请输入院系"
							  v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','faculty')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','faculty'))" :disabled="disabledObj['faculty_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','inoculation_time') || $check_field('add','inoculation_time') || $check_field('set','inoculation_time')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="接种时间" prop="inoculation_time">
					<el-date-picker :disabled="disabledObj['inoculation_time_isDisabled']" v-if="user_group === '管理员' || (form['teacher_vaccination_appointment_id'] && $check_field('set','inoculation_time')) || (!form['teacher_vaccination_appointment_id'] && $check_field('add','inoculation_time'))" id="inoculation_time"
						v-model="form['inoculation_time']" type="datetime" placeholder="选择日期时间">
					</el-date-picker>
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/teacher_vaccination_appointment/upload?",
				...
				form: {
					...
					"teacher_vaccination_appointment_id": 0, // ID
		...
		methods: {
			...
			get_obj_after(json, func){
				if(parseInt(this.form["inoculation_time"]) > 9999){
					this.form["inoculation_time"] = this.$toTime(parseInt(this.form["inoculation_time"]),"yyyy-MM-dd hh:mm:ss")
				}
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/teacher_vaccination_appointment/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_vaccination_appointment/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_vaccination_appointment/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_vaccination_appointment/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_vaccination_appointment/view','get');
